document.addEventListener('DOMContentLoaded', async () => {
  ...
  const creditViolationScreen = document.getElementById('creditViolationScreen');

  // Credit integrity check - must pass before anything else
  try {
    const verifyRes = await fetch('/api/system/verify');
    const verifyData = await verifyRes.json();
    if (!verifyData.valid) {
      // Credits tampered - block access
      loginContainer.classList.add('hidden');
      creditViolationScreen.classList.remove('hidden');
      return; // Stop all further logic
    }
  } catch (err) {
    // If server is unreachable, allow login attempt (will fail at login anyway)
